...
Concerns that would overload the implementation pass are factored into separate passes that run after it, each with a single mandate and a clean context. The implementation brief carries only the **non-retrofittable** constraints — the ones that shape the code and can't be cheaply added later (platform, types, test framework, allocation/lifetime, domain invariants). Everything **retrofittable** — formatting, lint, doc comments, naming polish, consistency — is deferred to a dedicated pass, where a fresh focused context does it better and cheaper than an implementation pass doing it distractedly.

Two kinds:

- **Mechanical** — a tool (formatter, linter) run deterministically. Cheap; make it a standard step.
- **Semantic** — an agent re-reads the finished code under a focused mandate (e.g. a documentation pass). Because articulating intent forces the reader to confront intent-vs-code mismatches, the semantic pass doubles as a lightweight correctness review and routinely surfaces bugs. It is distinct from the adversarial gate: the gate scrutinizes the *plan* before implementation; this scrutinizes the *code* after.

Discipline for the semantic pass: reconstruct intent from the code **skeptically** — pre-documenting everything during implementation tends to make the pass rubber-stamp and find less; rationale a reviewer genuinely can't infer is recorded at decision-time, not invented in the pass. Doc and style edits (non-behavioral) apply under show-diff; a behavioral fix the pass surfaces is a separate mutation step with tests, not a side effect. Scale to the change — a one-line fix gets a glance, a module gets the full pass — but the pass is a default phase, not an optional one.
