...
- **v0.7.4 (2026-06-04)** — Renamed `overlays/` to `aside/` and broadened it from repo-belonging symlink storage to a deliberate catch-all for workspace-scoped material without a better home (the symlink-storage role retained as one accepted kind), governed by a promotion rule and the kill criteria so it stays a waystation, not a graveyard.* *v0.7.3 — adds Post-implementation passes
