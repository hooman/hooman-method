...
Each concept in the guide has an eventual **tool binding** — the encoded form that lets the rule run automatically rather than by discipline. These are **candidate** bindings: each is adopted only when its own trigger fires. The mapping fills in as the method matures.
